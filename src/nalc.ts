...
import { ensureRegistryRuntime } from './registry/runtime';
import {
  addRegistryPackages,
...
yargs(process.argv.slice(2))
  ...
  .command({
    ...
    aliases: ['rm'],
    describe: 'Stop tracking selected local package overrides in the current project',
  ...
  .command({
    command: 'pass',
    describe: 'Restore the current project to normal dependencies and remove nalc state',
    builder: (y) => y.default(rcArgs).help(true),
    handler: async () => {
  ...
  .command({
    ...
    describe: 'Touch project inputs so editors rebuild the local dependency graph',
    handler: async (argv) => {
      const { refreshConsumerProjectGraph } = await import('./registry/project-graph');
